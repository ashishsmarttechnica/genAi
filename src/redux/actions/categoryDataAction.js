...
export const getCategoryDataAction = (page = 1, limit = 10, indexOrder = "", nameOrder = "", dateOrder = "", promptCountOrder = "") => {
    return async (dispatch) => {

        dispatch({ type: "CATEGORY_DATA_LOADING" });

        try {
            const res = await axiosInstance.get(`category/get-all?page=${page}&limit=${limit}&indexOrder=${indexOrder}&nameOrder=${nameOrder}&dateOrder=${dateOrder}&promptCountOrder=${promptCountOrder}`);
...
export const updateCategoryDataAction = (categoryId, toIndex) => {
    return async (dispatch, getState) => {
        ...
        try {
            ...
            if (res && res.data && res.data.success) {
                ...
                dispatch({ type: "CATEGORY_DATA_MOVE_ERROR", payload: { data: previousDataBackup } });
